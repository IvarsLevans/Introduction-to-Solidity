...
contract myGame {
    ...
    function addPlayer(string memory firstName, string memory lastName) private {
        Player memory newPlayer = players[msg.sender] = Player(msg.sender, Level.Novice, firstName, lastName, block.timestamp);
    ...
    function payOutWinners(address loserAddress) payable public {
        require(msg.sender == dealer, "Only the dealer can pay out the winnders");
